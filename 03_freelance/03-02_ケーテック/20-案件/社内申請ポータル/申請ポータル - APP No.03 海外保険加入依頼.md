...
- [ ] 渡航先マスタの初期データ（`config/masters/destination-master.csv`）を先方版に整備
      （※下記2026-09-23の依頼が通ると`DestinationMaster`自体が不要になる可能性あり）

> [!warning] 2026-09-23、先方から追加要望を受領（次回まとめ対応の方針、未着手）
> - 渡航者の下に氏名ローマ字(姓/名)・メールアドレス・生年月日・性別(プルダウン)を追加
> - 渡航先をLookup(`DestinationMaster`)からChoice(中国/中国以外のアジア/アメリカ/ヨーロッパ)の
>   固定4択へ戻す。2026-08-17にこちら発で表記ブレ対策としてLookup化した設計の巻き戻しにあたる
> - 「帰国日」の表示名を「帰宅日」に変更
>
> 詳細・下調べは [[30-プラットフォーム/spo/ver10/ver10 要望事項]] 参照。即対応せず、他の追加要望と
> まとめて次回(ver10想定)着手する方針
