...
> 用途は [[platform - 00 概要|統合コンソール（ryokuchaen-platform）]] の EC 機能（旧 [[platform - 09 受注機能（旧 EC Channel Console）|EC Channel Console]]。2026-09-15 に platform へ移行済み）で**スタブのまま止まっている Temu チャネルの実装判断**。
...
> [[platform - 09 受注機能（旧 EC Channel Console）]]（移行前の案件ノート）では Temu を「クロスモールの連携資料も見つからず、対応状況自体が不明」としていた。この調査で**出品者向けの公式 受注API は存在する**ことが確認できた。残る論点は次の3つ。
...
> 3. ~~**日本本土店の router ホストが未確認。** GLOBAL／US は判明しているが、日本はオンボーディング時に公式確認が必要（§7-7）~~ → **GLOBAL で確定**。公式「Endpoints and Request Method」の表で日本は GLOBAL（`openapi-b-global.temu.com`）に含まれる。EU 用の `openapi-b-eu.temu.com` もある（2026-09-16）

> [!success] 2026-09-16 実装した（公式ドキュメントに基づく。**実データ未照合**）
> 下表の差を受けて受注取得を実装した。MD5 署名は公式の署名例と一致することを確認済み。残る不確定要素は実データ側（店舗モデル・認可スコープ・金額 API が使えるか）で、判断が変わった点は次のとおり。
> - **配送先の氏名・住所は取りに行かない**（§5 の保存制約。画面は「(非公開)」）。MDC 投入で住所が要る段になったら改めて決める
> - **金額は `temu.order.amount.v2.query`** で補う（公式には日本・韓国サイトのみ対象。取れなければ 0 表示）
> - ホストと regionId は任意の設定項目にした（未入力なら GLOBAL・全リージョン）
> - トークン期限切れ（`3000034`）は「セラーセンターで再認可」と案内する。期限の事前通知・自動更新はしない

> [!warning] 2026-09-16 現行実装との突き合わせ（`ryokuchaen-platform` main `571d222` 時点・`lib/ec/clients/temu.ts`。移植元の `multi-channel-order-fetcher` も同じスタブ）※上の実装前の記録
...
> [[platform - 10 変換器構想とMDC出力|MDC出力]] で easyECS に住所ごと投入する構想や、Supabase に受注明細を保存する設計とは**緊張関係がある**。Temu をMDC投入対象に含める前に、保存の範囲（住所を持たない／期限付きで消す）を決める必要がある。
...
7. **日本の本土店のホスト**：~~日本本土店のrouterホストはオンボーディング時に公式確認が必要（要確認）~~ → **GLOBAL（`openapi-b-global.temu.com`）**。公式「Endpoints and Request Method」で US（`openapi-b-us`）／EU（`openapi-b-eu`）／GLOBAL（メキシコ・日本など）の3つと明記。注文照会の `regionId` は日本＝100（公式「Order Retrieve and Management」の列挙）
...
- [[platform - 09 受注機能（旧 EC Channel Console）]] — Temu を含む7チャネルの実装状況（移行前の案件ノート）
- [[platform - 00 概要]] — 移行先の統合コンソール
- [[Amazon - SP-API 受注リファレンス]] — 同じ形式で整理した Amazon 版
- [[platform - 10 変換器構想とMDC出力]] — Temu を MDC 投入対象に含める構想（PII保存方針と要調整）
- [[クロスモール（I'LL社）]] — クロスモールの Temu 対応は資料が見つかっておらず不明のまま
- [[_未確定事項]] — 店舗モデル・アプリ登録の確認事項
