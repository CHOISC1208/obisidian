...
| D10 | 画面とデータの切り方 | **画面の入口（ヘッダーのタブ・サイドバー・URL）はモジュール（ダッシュボード／受注／入出荷／帳票／マスタ）で切り、データの持ち主（スキーマ・`lib/`・権限キー）は業務（`inventory`／`ec`／`core`）で切る。** 月締めは「入出荷」。受注残ボードは当初「入出荷」→ **2026-09-17 に「ダッシュボード」を新設して移した** | #8・#9 |
...
|---|---|---|
| ダッシュボード（2026-09-17 追加） | `/dashboard` | ec（受注残ボード） |
| 受注 | `/orders` | ec |
| 入出荷 | `/logistics` | inventory（受注残ボードは 2026-09-17 にダッシュボードへ移した） |
...
| 項目 | 方針 | 由来 |
|---|---|---|
| 第1階層 | **ヘッダーのモジュールタブ**（ダッシュボード／受注／入出荷／帳票／マスタ。ダッシュボードは 2026-09-17 追加）。使える画面が1つも無いタブは出さない | デザイン指示書 |
