...
| スキーマ | `inventory`。`public` は [[受注機能（旧 EC Channel Console）|EC Channel Console]] が使用中のため分離 |
...
> **本案件の成果物ではなく [[受注機能（旧 EC Channel Console）]] 側の資産**だが、あわせて是正した。
>
...
- [x] ~~（別案件）`public` の SECURITY DEFINER ビュー5本の是正~~ → **2026-09-13 完了**（[[受注機能（旧 EC Channel Console）]]）
...
- [[受注機能（旧 EC Channel Console）]] — 同じSupabaseプロジェクトの `public` を使う別案件
- [[_未確定事項]] — 未決事項の集約先
