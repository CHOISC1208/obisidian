...
> メール＋パスワードに SAML SSO を足す案。SAML はプロジェクトで既定が無効、プランは Pro で条件を満たしている。現状の `proxy.ts` はコールバックを横取りするため除外が要り、権限が `auth.users.id` 基準であることが最大の論点。→ [[platform - 08 TrustLogin SSO 検討]]

> [!info] 2026-09-17：統合コンソールはメール＋パスワードに2段階認証（TOTP）を必須にした
> 本書の比較（統合前の2アプリ）は「パスワードだけ」の前提。統合コンソールでは SSO が入るまでの代わりに TOTP を入れた。Bearer のアクセスも2段階目まで済んだトークンが要る → [[platform - 05 統合方針（決定事項と設計）]] 3章
