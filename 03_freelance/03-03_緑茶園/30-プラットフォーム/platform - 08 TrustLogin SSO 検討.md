...
> **手順の正本は [[platform - 08a TrustLogin SSO 導入手順]]**（SP情報の表・先方への依頼文・`sso add` 以降の作業・検証手順）。ここに残すのは**前提が合っているかの判断**だけ。
...
> - 依頼文は [[platform - 08a TrustLogin SSO 導入手順]] に用意済み。**IdP メタデータが返ってくるまでこちらの作業は無い**
...
| | 内容 |
|---|---|
| 依頼する相手 | TrustLogin の管理画面を操作できる人〔未確定。→ [[_未確定事項]]〕 |
...
| 併せて確認すること | 管理者が誰か／契約プランで SAML アプリが使えるか／ポータルからのアイコン起動を使うか → [[_未確定事項]] |

依頼文の実物（URL・値を含む完全版）は [[platform - 08a TrustLogin SSO 導入手順]]。**値の二重管理を避けるため、ここには転記しない。**
