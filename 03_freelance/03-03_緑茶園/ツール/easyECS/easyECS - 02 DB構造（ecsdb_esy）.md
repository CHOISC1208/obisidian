...
- `all_total`（請求合計）が 0 の受注が、楽天Pay で年に約1,000件、Wowma! で約400〜500件ある。**いずれも商品の金額はあり、ポイントで全額払われた受注**（下の式で確認）

### 受注の金額の列の関係（2025年の全受注で確認）

| 式 | 成り立つ割合 |
|---|---|
| `t_sell.out_tax` ＝ 明細 `t_sell_goods.out_tax` の合計 | 楽天Pay・Amazon・Yahoo!・Wowma! で100%（MDC は 122件中116件） |
| `out_tax` ＋ `tax` ＝ `in_tax` | 100% |
| `all_total` ＝ `in_tax` ＋ `carriage` ＋ `collect_commission` − `point` − `coupon` | 100% |
| `t_sell.carriage` ＝ 配送先 `t_sell_delivery.carriage` の合計 | 100% |

→ `out_tax` は**税抜・送料なし・ポイントとクーポンを引く前**の商品代金。`all_total` はお客さんの支払額で、売上ではない。

> [!note] ダッシュボードの「売上」の定義（2026-09-17 ちぇる判断。先方未確認）
> **税抜・送料なし**とし、`t_sell.out_tax`（明細は `t_sell_goods.out_tax`）で集計する。理由は、送料の有無やモールの負担するポイントで商品の売れ行きが上下して見えないようにするため。
> クーポン（2025年 約234万円・税込）は売上から引かずに別の数字として出す。**クーポンの原資が自社かモールかは DB からは分からない**ので、先方に確認する。
