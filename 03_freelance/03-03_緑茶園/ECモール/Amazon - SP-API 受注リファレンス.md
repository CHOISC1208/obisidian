...
> 用途は [[platform - 00 概要|統合コンソール（ryokuchaen-platform）]] の EC 機能（旧 [[EC Channel Console - 00 概要|EC Channel Console]]。2026-09-15 に platform へ移行済み）の受注取り込み設計と、PowerAutomate 置換（Amazon純正フォーマットCSV出力、→ [[EC Channel Console - 変換器構想とMDC出力]]）の実現可否の判断。
...
> [!tip] 緑茶園の文脈では
> 送り状番号のモール返却は easyECS の「出口」側の価値（→ [[EC Channel Console - 00 概要]]）。統合コンソールが担うのは入口なので、confirmShipment / Feeds API は当面スコープ外。
...
- [[EC Channel Console - 00 概要]] — Amazon を含む7チャネルの実装状況（移行前の案件ノート）
- [[platform - 00 概要]] — 移行先の統合コンソール
- [[各モール API認証情報の取得手順]] — `AMAZON_LWA_APP_ID` 等の取得手順（先方配布用）
- [[EC Channel Console - 変換器構想とMDC出力]] — PowerAutomate置換（純正フォーマットCSV出力）の構想
